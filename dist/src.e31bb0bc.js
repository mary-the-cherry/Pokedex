...
var btnPaginationItem3 = document.querySelector("#pagination-item-three");
...
function fetchPokemonList() {
  ...
  fetch('https://pokeapi.co/api/v2/pokemon?limit=600').then(function (response) {
    ...
    allpokemons.results.forEach(function (pokemon) {
      ...
      pokemonListeName.innerHTML = "<p> ".concat(jsUcfirst(pokemon.name), " </p>");
      pokemonListeName.addEventListener("click", function () {
...
function createPokemonCard(pokemondata, pokemoncard) {
  ...
  cardTitle.innerHTML = "<h3>".concat(jsUcfirst(pokemondata.name), "</h3>");
  cardBody.appendChild(cardTitle); // add data to card

  var cardText = document.createElement("div");
  cardText.className = "card-text";
  cardText.innerHTML = "<p> ID: <strong>".concat(pokemondata.id, "</strong> &nbsp &nbsp Order: <strong>").concat(pokemondata.order, "</strong> &nbsp &nbsp\n                                    Base-Exp.: <strong>").concat(pokemondata.base_experience, "</strong> </p>");
...
function getPokemonImage(pokemondata, cardImg, card) {
  ...
  fetch(pokemondata.sprites.other.dream_world.front_default).then(function (response) {
...
function createTypeField(pokemondata, card) {
  ...
  pokemondata.types.forEach(function (pokemontype) {
    pokemonType.innerHTML += "<span> ".concat(jsUcfirst(pokemontype.type.name), " </span>");
...
function createFullCard(pokemon) {
  ...
  document.querySelector("#pokemoncardFullNumbers").innerHTML = "ID: <strong>".concat(pokemon.id, "</strong> &nbsp &nbsp Order: <strong>").concat(pokemon.order, "</strong> &nbsp &nbsp\n                                    Base-Exp.: <strong>").concat(pokemon.base_experience, "</strong>"); //loop through all types and add to a array, add array to HTML document

  var types = [];
  pokemon.types.forEach(function (type) {
    return types.push(jsUcfirst(type.type.name));
  });
  document.querySelector("#pokemoncardFullType").innerHTML = "<strong>Type:</strong> " + types.join(', '); //loop through all abilities and add to a array, add array to HTML document

  var abilities = [];
  pokemon.abilities.forEach(function (ability) {
    return abilities.push(jsUcfirst(ability.ability.name));
  });
  document.querySelector("#pokemoncardEffects").innerHTML = "<strong>Abilities:</strong> " + abilities.join(', '); //loop through all stats and add to a array, add array to HTML document

  var stats = [];
  pokemon.stats.forEach(function (stat) {
    return stats.push(jsUcfirst(stat.stat.name));
  });
  document.querySelector("#pokemoncardFullKampf").innerHTML = "<strong>Fightskills:</strong> " + stats.join(', '); //add height and weight

  document.querySelector("#pokemoncardFullWeight").innerHTML = "<strong>Weight:</strong> ".concat(pokemon.weight, " g");
  document.querySelector("#pokemoncardFullHeight").innerHTML = "<strong>Height:</strong> ".concat(pokemon.height, " cm"); //add image as blob to card and make card visible

  fetch(pokemon.sprites.other.dream_world.front_default).then(function (response) {
    return response.blob();
  }).then(function (blob) {
    var pokeImg = URL.createObjectURL(blob);
...
function searchPokemon() {
  ...
  console.log('search');
  var inputPokemonValue = inputPokemonSearch.value.toLowerCase().replace(/\s/g, ''); //to lower case and without whitespaces
  ...
  if (inputPokemonValue === "") {
    ...
    document.querySelectorAll(".pagination li").forEach(function (paginationItem) {
    ...
    globalPokemonlist.filter(function (pokemon) {
...
}, false); //Eventlistener Pagination Page : Page 2 --> Loads the second 20 pokemons
...
}, false); //Eventlistener Pagination Page : Page 3 --> Loads the third 20 pokemons
...
}, false); //Eventlistener Pagination Next: Jumps three pages back and adjusts the Pagination Items 
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "59319" + '/');
